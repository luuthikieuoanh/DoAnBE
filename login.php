...
if (!empty($_POST['email']) && !empty($_POST['password'])) {
	$emailDB = '';
	$passwordDB = '';
	//lấy chuỗi email từ database
	foreach ($user->getEmail($_POST['email']) as $key => $value) {
		foreach ($value as $key1 => $value1) {
			$emailDB=$value1;
		}
	}
	//lấy chuỗi password từ database
	foreach ($user->getPassword($_POST['email']) as $key => $value) {
		foreach ($value as $key1 => $value1) {
			$passwordDB=$value1;
		}
	}
					...
					<div class="row">
						<div class="col-sm-6">
							<div class="well">
								...
								<a href="https://demo.templatetrip.com/Opencart/OPC01/OPC009/OPC04/index.php?route=account/register" class="btn btn-primary">Continue</a>
							</div>
						</div>
						<div class="col-sm-6">
							<div class="well">
